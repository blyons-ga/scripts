...
* Bruce Lyons   | 2018-04-20 | v1.2     | Updated to add in checking for McAfee Encryption              *
* ===================================================================================================== *
*                                                                                                       *
*********************************************************************************************************
.SYNOPSIS
...
#region Function Get-McAfeeEncryptionStatus
<#
.SYNOPSIS
    This Function is used the get the McAfee Encryption Protection Status.
.DESCRIPTION
    This Function is used the get the McAfee Encryption Protection Status.
.PARAMETER DriveLetter
    Drive Letter to check for BitLocker Status. Optional Parameter.
#>
Function Get-McAfeeEncryptionStatus {

$ErrorActionPreference = "stop"
$registryPath = "HKLM:\SOFTWARE\Wow6432Node\McAfee EndPoint Encryption\MfeEpePc\Status"
$Name = "Version"
$value = "1"

Try {
 $McAfeeEncrypt = Get-ItemProperty -Path "HKLM:\SOFTWARE\Wow6432Node\McAfee EndPoint Encryption\MfeEpePc\Status" -Name Activated
 If ($McAfeeEncrypt.Activated -eq "Yes") {
    "MCAFEE ENCRYPTION ENABLED"
 } Else {
    "MCAFEE ENCRYPTION DISABLED"
 }
}
...
#>
    ...
    Try {
        ...
        Get-CimInstance  -Namespace 'root\CIMV2\Security\MicrosoftVolumeEncryption'  -ClassName 'Win32_EncryptableVolume' -ErrorAction 'Stop' | ForEach-Object {
            $McAfeeEncrypt = Get-McAfeeEncryptionStatus
            If ($McAfeeEncrypt -ne "") {
                $ResultProps = [ordered]@{
                    'Drive' = $_.DriveLetter
                    'ProtectionStatus' = $McAfeeEncrypt
            } 
            } Else { 
